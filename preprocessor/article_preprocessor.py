...
class ArticlePreprocessor:
    ...
    def stopwords_remover(self, selected_token : list) -> str:
        ...
        # replace() 메서드를 사용해 잘못된 띄어쓰기를 수정
        preprocessed_text = preprocessed_text.replace("불 확실", "불확실").replace("불 균형", "불균형").replace("불 호황", "불호황")
        preprocessed_text = preprocessed_text.replace("입 주량", "입주량").replace("종 부세", "종부세")
    ...
    def csv_exporter(self, result_df : pd.DataFrame):
        ...
        저장되는 CSV 파일의 이름은 'naver_news_article_preprocessed.csv'입니다.
        """
        ...
        result_df.to_csv(export_dir + "naver_news_article_preprocessed.csv", index = False, encoding = "utf-8-sig")
